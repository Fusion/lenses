...
function Accordian(d,s,tc){
	...
	l=$accordion(d).getElementsByTagName('div');
	c=[];
	...
	for(i=0;i<l.length;i++){
		...
		if(h.substr(h.indexOf('-')+1,h.length)=='header'){
			d=$accordion(h.substr(0,h.indexOf('-'))+'-content');
			...
			h.onclick = function(){
				for(i=0;i<this.c.length;i++){
					...
					if((n+'-header')==this.id){
						ex($accordion(n+'-content'));
						n=$accordion(n+'-header');
						cc(n,'__');
						n.className=n.className+' '+n.tc;
					}else{
						cl($accordion(n+'-content'));
						cc($accordion(n+'-header'),'');
					}
				}
			}
