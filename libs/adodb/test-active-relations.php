<?php
	...
	$db->Execute("insert into songs (recordid, name, artist) values(1,'No Hiding Place', 1)");
	$db->Execute("insert into songs (recordid, name, artist) values(2,'American Gangster Time', 1)");
	...
	echo "\n\n---------------------------------------------------------------------------\n";
	echo "artist->Load('artists.artistuniqueid=1') [Join Method]\n";
	echo "---------------------------------------------------------------------------\n";
	$ar1 = new Artist();
	$ar1->Load('artists.artistuniqueid=1');
	var_export($ar1);
	echo (ar_assert($ar1, "'name' => 'Elvis Costello'")) ? "[OK] Found Elvis Costello\n" : "[!!] Find failed\n";
	echo (ar_assert($ar1, "'name' => 'No Hiding Place'")) ? "[OK] Found relation: song\n" : "[!!] Missing relation: song\n";


	echo "\n\n---------------------------------------------------------------------------\n";
	echo "song->Load('recordid=1') [Join Method]\n";
	echo "---------------------------------------------------------------------------\n";
	$so1 = new Song();
	var_export($so1);
	$so1->Load('songs.recordid=1');
	var_export($so1);
	echo (ar_assert($so1, "'name' => 'No Hiding Place'")) ? "[OK] Found song\n" : "[!!] Find failed\n";

	echo "\n\n---------------------------------------------------------------------------\n";
	echo "artist->Find('id=1' ... ADODB_JOIN_AR) [Join Method]\n";
	echo "---------------------------------------------------------------------------\n";
	$ar1 = new Artist();
	$ar1->Find('artists.artistuniqueid=1', false, false, array('loading' => ADODB_JOIN_AR));
	var_export($ar1);
	echo (ar_assert($ar1, "'name' => 'Elvis Costello'")) ? "[OK] Found Elvis Costello\n" : "[!!] Find failed\n";
	echo (ar_assert($ar1, "'name' => 'No Hiding Place'")) ? "[OK] Found relation: song\n" : "[!!] Missing relation: song\n";
	...
	echo "\n\n---------------------------------------------------------------------------\n";
	echo "song->Find('id=1' ... ADODB_JOIN_AR) [Join Method]\n";
	echo "---------------------------------------------------------------------------\n";
	$so1 = new Song();
	var_export($so1);
	$so1->Find('songs.recordid=1', false, false, array('loading' => ADODB_JOIN_AR));
	var_export($so1);
	echo (ar_assert($so1, "'name' => 'No Hiding Place'")) ? "[OK] Found song\n" : "[!!] Find failed\n";
